...
<body>
    <div class="header">
        <h2>REKAP LAPORAN BULANAN</h2>
        @if($user->isDesa())
            <p>Desa: {{ $user->desa->nama_desa }}</p>
        @elseif($user->isKecamatan())
            <p>Kecamatan: {{ $user->kecamatan->nama_kecamatan }}</p>
        @endif
        <p>Periode: {{ $startDate->isoFormat('MMMM Y') }}</p>
    </div>

    @foreach($groupedReports as $daiId => $daiReports)
        @php 
            $firstReport = $daiReports->first();
            $totalKegiatan = $daiReports->count();
            $kegiatanByType = $daiReports->groupBy('type');
        @endphp
        ...
        <div class="monthly-summary">
            ...
            <table>
                <tr>
                    ...
                    <th>Rincian per Tipe</th>
                </tr>
                <tr>
                    <td>{{ $totalKegiatan }} Kegiatan</td>
                    <td>
                        @foreach($kegiatanByType as $type => $reports)
                            {{ $type }}: {{ $reports->count() }} Kegiatan<br>
                        @endforeach
                    </td>
                </tr>
            </table>
